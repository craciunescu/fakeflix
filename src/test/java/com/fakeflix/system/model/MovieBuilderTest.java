...
public class MovieBuilderTest {
    /*
     * @Test void shouldInitialize() { final Author author = new Author("John",
     * "Doe");
     * 
     * final Media movie = new MediaBuilder().name("FILM1").tags(List.of(Tag.ACTION,
     * Tag.HORROR)) .author(author).build();
     * 
     * assertThat(movie).isNotNull(); assertThat(movie.getAuthor()).isNotNull();
